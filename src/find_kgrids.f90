...
CONTAINS
  ...
  SUBROUTINE find_grid(lat_vecs, kpd, B_vecs, at, offset, find_offset, best_grid, &
    ...
    real(dp), allocatable :: grids(:,:,:), ratio(:), grid_offsets(:,:)
    ...
    if (kpd > 500) then
       if ((lat_id==3) .or. (lat_id==5) .or. (lat_id==1)) then
          ...
          do while ((count < 5) .and. (i <= 10))
             a_kpd = c_kpd(i)
             if (lat_id==3) then
                call sc_3(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, offset, find_offset, lat_id, &
                     best_offset, temp_hnfs, temp_grid, temp_nirr, temp_nhnfs, symm_flag, &
                     eps_=eps)
             else if (lat_id==5) then
                call bcc_5(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, offset, find_offset, lat_id, &
                     best_offset, temp_hnfs, temp_grid, temp_nirr, temp_nhnfs, symm_flag, &
                     eps_=eps)
             else if (lat_id==1) then
                call fcc_1(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, offset, find_offset, lat_id, &
                     best_offset, temp_hnfs, temp_grid, temp_nirr, temp_nhnfs, symm_flag, &
                     eps_=eps)
             end if
          ...
          do while (found_min .eqv. .False.)
             call tric_31_44(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, mult, offset, find_offset, lat_id, &
          ...
          do while ((count < 5) .or. ((count >= 5) .and. (a_kpd-kpd < s_range)))
             if ((lat_id==2) .or. (lat_id==4)) then
                call rhom_4_2(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, offset, &
                     find_offset, lat_id, best_offset, temp_hnfs, temp_grid, temp_nirr, &
                ...
                call bct_6_7_15_18(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, &
                     offset, find_offset, lat_id, best_offset, temp_hnfs, temp_grid, &
                     temp_nirr, temp_nhnfs, symm_flag, eps_=eps)
             else if (lat_id==8) then
                call bco_8(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, offset, &
                     find_offset, lat_id, best_offset, temp_hnfs, temp_grid, temp_nirr, &
                     temp_nhnfs, symm_flag, eps_=eps)
             else if (lat_id==9 .or. lat_id==24) then
                call rhom_9_24(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, offset, &
                     find_offset, lat_id, best_offset, temp_hnfs, temp_grid, temp_nirr, &
                     temp_nhnfs, symm_flag, eps_=eps)
                ...
                call basecm_10_14_17_27_37_39_41(a_kpd, No, Nu, Co, Cu, O, lat_vecs, &
                     B_vecs, at, offset, find_offset, lat_id, best_offset, temp_hnfs, &
                     temp_grid, temp_nirr, temp_nhnfs, symm_flag, eps_=eps)
             else if (lat_id==11) then
                call st_11(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, offset, &
                     find_offset, lat_id, best_offset, temp_hnfs, temp_grid, temp_nirr, &
                     temp_nhnfs, symm_flag, eps_=eps)
             else if (lat_id==12) then
                call hex_12(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, offset, &
                     find_offset, lat_id, best_offset, temp_hnfs, temp_grid, temp_nirr, &
                     temp_nhnfs, symm_flag, eps_=eps)
                ...
                call fco_16(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, offset, &
                     find_offset, lat_id, best_offset, temp_hnfs, temp_grid, temp_nirr, &
                     temp_nhnfs, symm_flag, eps_=eps)
             else if (lat_id==19) then
                call bco_19(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, offset, &
                     find_offset, lat_id, best_offset, temp_hnfs, temp_grid, temp_nirr, &
                     temp_nhnfs, symm_flag, eps_=eps)
                ...
                call st_21(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, offset, &
                     find_offset, lat_id, best_offset, temp_hnfs, temp_grid, temp_nirr, &
                     temp_nhnfs, symm_flag, eps_=eps)
             else if (lat_id==22) then
                call hex_22(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, offset, &
                     find_offset, lat_id, best_offset, temp_hnfs, temp_grid, temp_nirr, &
                     temp_nhnfs, symm_flag, eps_=eps)
             else if (lat_id==23) then
                call baseco_23(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, offset, &
                     find_offset, lat_id, best_offset, temp_hnfs, temp_grid, temp_nirr, &
                     temp_nhnfs, symm_flag, eps_=eps)
             else if (lat_id==26) then
                call fco_26(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, offset, &
                     find_offset, lat_id, best_offset, temp_hnfs, temp_grid, temp_nirr, &
                     temp_nhnfs, symm_flag, eps_=eps)
             else if (lat_id==28) then
                call basecm_28(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, offset, find_offset, lat_id, &
                     best_offset, temp_hnfs, temp_grid, temp_nirr, temp_nhnfs, symm_flag, &
                     eps_=eps)
             else if ((lat_id==29) .or. (lat_id==30)) then
                call basecm_29_30(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, &
                     offset, find_offset, lat_id, best_offset, temp_hnfs, temp_grid, temp_nirr, temp_nhnfs, &
                     symm_flag, eps_=eps)
             else if (lat_id==32) then
                call so_32(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, offset, find_offset, lat_id, &
                     best_offset, temp_hnfs, temp_grid, temp_nirr, temp_nhnfs, symm_flag, &
                     eps_=eps)
             else if (lat_id==33) then
                call sm_33(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, offset, find_offset, lat_id, &
                     best_offset, temp_hnfs, temp_grid, temp_nirr, temp_nhnfs, symm_flag, &
                     eps_=eps)
             else if (lat_id==34 .or. lat_id==35) then
                call sm_34_35(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, offset, find_offset, lat_id, &
                     best_offset, temp_hnfs, temp_grid, temp_nirr, temp_nhnfs, symm_flag, &
                     eps_=eps)
             else if (lat_id==36) then
                call baseco_36(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, offset, find_offset, lat_id, &
                     best_offset, temp_hnfs, temp_grid, temp_nirr, temp_nhnfs, symm_flag, &
                     eps_=eps)
             else if (lat_id==40) then
                call baseco_40(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, offset, find_offset, lat_id, &
                     best_offset, temp_hnfs, temp_grid, temp_nirr, temp_nhnfs, symm_flag, &
                     eps_=eps)
             else if (lat_id==42) then
                call bco_42(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, offset, find_offset, lat_id, &
                     best_offset, temp_hnfs, temp_grid, temp_nirr, temp_nhnfs, symm_flag, &
                     eps_=eps)
             else if (lat_id==43) then
                call basecm_43(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, offset, find_offset, lat_id, &
                     best_offset, temp_hnfs, temp_grid, temp_nirr, temp_nhnfs, symm_flag, &
                     eps_=eps)
             end if
             if  (temp_nhnfs > 1 .and. any(temp_hnfs > 0)) then
       ...
       do while ((a_kpd-kpd < s_range) .and. (count < 5))
          call tric_31_44(a_kpd, No, Nu, Co, Cu, O, lat_vecs, B_vecs, at, mult, offset, &
               find_offset, lat_id, best_offset, temp_hnfs, temp_grid, temp_nirr, temp_nhnfs, &
               symm_flag, eps_=eps, space_group_=size(sg_ops, 3))
          if (temp_nhnfs > 1 .and. any(temp_hnfs > 0)) then
             count = count + 1
             sp_hnfs(:,:,count) = temp_hnfs(:,:,1)
             grids(:,:,count) = temp_grid
